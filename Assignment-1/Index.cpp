                ...
                if(splitNecessary(current->numkeys + 1, 'c') != 1){
                    ...
                    /*
                      ...
                      */
                      ...
                      int splitposn = 0;
                      splitposn = checkDuplicatesInNode(current,key,i,payload);
                      
                      //cout<< "this is the split position "<<splitposn<<endl;
                      //cout<< "this is the insert position "<<i<<endl;
                      if(splitposn>current->numkeys)
                      return 0;  // this condition signifies an overflow of duplicate nodes
                      //incase of normal condition
                      //cout<<"splitted at "<<splitposn;
						handleLeafSplit(key, payload, &current,accessPath,height,i,splitposn);
...
int checkDuplicatesInNode(TreeNode * current,byte *key,int insPos,byte *payload)
	{
		...
		int m=0;
		int k=0;
		int j;int duplicate = 0;
		for (j = 0; j < current->numkeys; j++) {
                
                current->getKey(keytype, nodekey, j);
				int result = compare(nodekey, key, keytype);
				//cout<<"result : "<<result;
		...
		if(duplicate){//key is duplicate
					//cout<< j<< " duplicate at key posn \n";
					if(j==0){ //split at end of similar elements 
						//cout<<"reached line 197"<<endl;
						
						m = checkForDuplicatesForCurrentKey(current,key);
						return m;
					}
					else{
						//split signal at this position;
						return j;
					}
		...
		else{ // key is not duplicate, so either there are no duplicates, or there was presexisting duplicates before split
			
			
			  int splitAt = checkPreexistingDuplicates(current,insPos);
				return splitAt;
	...
	int checkPreexistingDuplicates(TreeNode * current,int insPos)
	{
		  char nodekey[keylen(&keytype)];      char nextnodekey[keylen(&keytype)];
		  
		    int left;int right;//right signifies number of consecutive pairs match
		    left = 0; right = 0;
				
					for (int i = 0; i < current->numkeys-1; i++) {
					
						current->getKey(keytype, nodekey, i);
						current->getKey(keytype, nextnodekey, i+1);
                
						if((compare(nodekey,nextnodekey, keytype) == 0) ){
			...
			//cout<<"left pointer: "<<left<<"  right pointer:  "<<right<<endl;
			
			if(left>right) {
			
			//then no duplicate and so split at size/2
			return ((current->numkeys)/2);
			}
			else{
			
			//there is some duplicate starts at left and ends at right.
			int returnVal = 0;
			
			
				if(left == 0)
				{
					//split at position right.
					//right value means number of pair match,
					//right = 2 means 0 to 2 nodes are same so split after 3rd node
					returnVal = (right+1);
					
					if(insPos<returnVal) returnVal = 1;
				}
				else{
					
					//split at left, left = 1 means send split value as 1 
					returnVal = left;
					if(insPos<returnVal) returnVal = (left+1);
					
				}
			//if the newly inserted value insertion position is less than the 
			return returnVal;
			}
...
int checkForDuplicatesForCurrentKey(TreeNode * current,byte *key)
		{
		  char nodekey[keylen(&keytype)];
			...
			int j=0;
					...
					for (j = 0; j < current->numkeys; j++) {
						...
						if((compare(nodekey,key, keytype) != 0) ){
						//cout<< "value at break"<< j <<endl;
						break;
							
						}
					}
					
			return j+1;		//adding 1 as the new node after insertion will change the last position
			
		}
	
	void showAll(TreeNode * current)
	{
		TreeNode * temp;int i;
		char nodekey[keylen(&keytype)];
		  for (i = 0; i < current->numkeys; i++) {
                current->getKey(keytype, nodekey, i);
                cout<<Utils::getIntForBytes(nodekey);cout<<",";
    ...
    LookupIter* find(char key[]) {
		...
		//showAll(current);
            ...
            if (current->flag == 'c') {
                if (isLesser != 0)	//key not found, return empty iterator
                    return emptyResult ;

                
                else if(isLesser==0)//key found, return iterator
                {
				
				 LookupIter* res = new LookupIter(key,keytype,current,i-1,payloadlen);// asigning i-1 to be complatible with implementation of LookupIter
...
char *keyN = (char *) calloc(8, 1);
    ...
    while(!(testValsSet.empty())){
        ...
        while(res->hasNext()){
            res->next();
            res->get(payL);
            //cout<<"Value received as payload: "<<Utils::getIntForBytes(payL)<<endl;
            count++;
        }
        printf("res %d : %d\n", curVal, count);
        delete(res);
